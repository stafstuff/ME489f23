...
void runSolver( int n, int rank, int size ){
  ...
  for ( int i = 0; i <= n + 1; i++ ){
    ...
    x[i]=rank*(x_max-x_min)/size+dx*(i-1);
...
// In multi-processor mode
    ...
    // write out the x coordinates for display.
    {
      double *x_global=(double*)malloc((size*n+1)*sizeof(double));

      MPI_Allgather(&x[1],n,MPI_DOUBLE,x_global,n,MPI_DOUBLE,MPI_COMM_WORLD);
      x_global[size*n]=x_max;
    ...
    {
      double *q_global=(double*)malloc((size*n+1)*sizeof(double));

      MPI_Allgather(&q[1],n,MPI_DOUBLE,q_global,n,MPI_DOUBLE,MPI_COMM_WORLD);

      q_global[size*n]=initial_condition(x_max,time);
 ...
 // Record the starting time.
  ...
  // In single processor mode, add current solution data to output file.
    ...
    if (size!=1 && OUT==1){
        ...
        double *q_global=(double *)malloc((n*size+1)*sizeof(double));

        MPI_Allgather(&q[1],n,MPI_DOUBLE,q_global,n,MPI_DOUBLE,MPI_COMM_WORLD);

        q_global[n*size] = boundary_condition ( x_max, time_new );
