...
start = omp_get_wtime();
  for(int step = 0; step<Nsteps; step++){
    ...
    if(step%Noutput == 0){
      ...
      // IF FRAME=100
      if(frame==100){
        infinityNorm(advc.q,&advc.msh);
      }
      // PUT SERIES,CRITICAL REGION, ATOMIC OPERATION, REDUCTION FOR MAX Q VALUE HERE
      sprintf(fname, "test_%04d.csv", frame++);
      solverPlot(fname, &advc.msh, advc.q);
...
/* ************************************************************************** */
void infinityNorm(double *Q,mesh_t *msh){
  omp_set_num_threads(12);
printf("-Infinity Norm Calculations-------\n");
double start; 
double end; 
start = omp_get_wtime();
  // SERIES 
...
printf("Time Elapsed : %f\n",end-start);
  ...
  max_q=0;
  #pragma omp parallel for shared(max_q)
  for(int n=0;n<msh->Nnodes;n++){
    #pragma omp critical
    {
    if(fabs(Q[n])>max_q){
...
printf("-Critical Results ------------------\n");
printf("Infinity Norm: %lf\n",max_q);
printf("Time Elapsed : %f\n",end1-start1);
  // REDUCTION 
 double start2 = omp_get_wtime();
  max_q=0;
#pragma omp parallel for reduction(max:max_q)
 for(int n=0;n<msh->Nnodes;n++){
    if(fabs(Q[n])>max_q){
      max_q=Q[n];
    }
...
for (int n = 0; n < msh->Nnodes; n++) {
    ...
    max_q = fmax(max_q, fabs(Q[n]));
}
}
double end3 = omp_get_wtime();
printf("-Atomic Results ------------------\n");
printf("Infinity Norm: %lf\n", max_q);
printf("Time Elapsed : %f\n", end3 - start3);
}
/* ************************************************************************** */
